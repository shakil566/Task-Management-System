                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ url('admin/task/create') }}" class="btn btn-sm btn-info float-right"><i class="fas fa-plus"></i> @lang('english.CREATE_NEW')</a>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="dataTable" class="table table-bordered table-striped">
                                <thead>
                                    <tr class="text-center">
                                        ...
                                        <th>@lang('english.PROJECT_CODE')</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td>
                                            {{ Form::open(['url' => 'admin/task/' . $value->id, 'id' => 'delete']) }}
                                            {{ Form::hidden('_method', 'DELETE') }}
                                            <button type="button" class="btn btn-xs btn-primary assign-task tooltips" data-toggle="modal" title="Assign task to Teammates" data-id="{{ $value->id }}" data-project-id="{{ $value->project_id }}" data-target="#AssignTaskModal">
                                                <i class="fa fa-plus-circle"></i>
                                            </button>
                                            <a class='btn btn-primary btn-xs' href="{{ URL::to('admin/task/' . $value->id . '/edit') }}" title="{{ trans('english.EDIT') }}">
                                                <i class='fa fa-edit'></i>
                                            </a>
                                            <button class="btn btn-danger btn-xs" type="submit" title="{{ trans('english.DELETE') }}" data-placement="top" data-rel="tooltip" data-original-title="Delete">
...
<script type="text/javascript">
    $(document).on('click', '.assign-task', function() {
        ...
        $.ajax({
            url: "{{ URL::to('admin/task/assigntask') }}",
            type: 'POST',
            dataType: 'json',
            headers: {
    ...
    $(document).on("click", ".saveAssignTask", function() {
        ...
        $.ajax({
            url: "{{ URL::to('admin/task/saveAssigntask') }}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
